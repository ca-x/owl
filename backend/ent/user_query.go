...
func (_q *UserQuery) QueryDictionaries() *DictionaryQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, user.DictionariesTable, user.DictionariesColumn),
		)
...
func (_q *UserQuery) WithDictionaries(opts ...func(*DictionaryQuery)) *UserQuery {
	query := (&DictionaryClient{config: _q.config}).Query()
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withDictionaries; query != nil {
		if err := _q.loadDictionaries(ctx, query, nodes,
			func(n *User) { n.Edges.Dictionaries = []*Dictionary{} },
			func(n *User, e *Dictionary) { n.Edges.Dictionaries = append(n.Edges.Dictionaries, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadDictionaries(ctx context.Context, query *DictionaryQuery, nodes []*User, init func(*User), assign func(*User, *Dictionary)) error {
	...
	query.withFKs = true
	query.Where(predicate.Dictionary(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.DictionariesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_dictionaries
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_dictionaries" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_dictionaries" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
